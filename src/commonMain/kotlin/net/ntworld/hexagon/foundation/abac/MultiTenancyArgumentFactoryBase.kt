...
import net.ntworld.hexagon.foundation.validator.MultiTenancyValidator
...
abstract class MultiTenancyArgumentFactoryBase<out A : MultiTenancyArgument> :
    ...
    override fun make(builder: AuthorizableArgumentBuilderBase): A {
        ...
        this.errors.clear()
        if (MultiTenancyValidator.validate(data, errors) && this.validate(data)) {
            return this.build(
    ...
    private fun makeSubject(data: ArgumentBuilderData): MultiTenancy {
        ...
        if (null === subject) {
            val userId = data.getValue<String?>(BuilderKey.USER_ID.key)
            if (null === userId) {
                return guestOf(data.getValue(BuilderKey.TENANT_ID.key))
            }
            return userOf(data.getValue(BuilderKey.TENANT_ID.key), userId)
        }
